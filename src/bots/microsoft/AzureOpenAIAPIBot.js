...
export default class AzureOpenAIAPIBot extends LangChainBot {
    ...
    async checkAvailability() {
      if(!store.state.azureOpenaiApi.azureApiKey || !store.state.azureOpenaiApi.azureApiInstanceName || !store.state.azureOpenaiApi.azureOpenAIApiDeploymentName 
        || !store.state.azureOpenaiApi.azureOpenAIApiVersion) {
        this.constructor._isAvailable = false;
      } else {
        const chatModel = new ChatOpenAI({
          azureOpenAIApiKey: store.state.azureOpenaiApi.azureApiKey,
          azureOpenAIApiInstanceName: store.state.azureOpenaiApi.azureApiInstanceName,
          azureOpenAIApiDeploymentName: store.state.azureOpenaiApi.azureOpenAIApiDeploymentName,
